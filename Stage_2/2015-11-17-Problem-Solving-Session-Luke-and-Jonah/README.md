...
After a bit of difficulty and testing, Jonah starts out by getting the grid into a manageable data structure:
```python
...
for n in grid_list:
    if count < 20:
        ...
        master_array.append(newlist)
        newlist = [int(n)]
        count = 1
master_array.append(newlist)
```
This will make a 2d array which represents the original grid of numbers!  You can see ths by printing it out:
...
Now we can start tackling the main part of the problem!  We'll need to go through every possible direction for every number.  There are 8 directions from every number: up, down, right, left, up-right, down-right, up-left, down-left.  4 of these overlap if we go through every number (up with down, right with left, up-right with down-left, up-left with down-right).  We can therefore right four sets of nested loops in order to solve the problem:

```python
# Go through the data structure to located the 
...
for row in range(20):
    for column in range(17):
        horiz = 1
        for i in range(4):
            horiz *= master_array[row][column + i]
        if horiz >= max_product:
            max_product = horiz
...
for row in range(17):
    for column in range(20):
        vert = 1
        for i in range(4):
            vert*=master_array[row + i][column]
        if vert >= max_product:
            max_product = vert
...
for row in range(17):
    for column in range(17):
        diag = 1
        for i in range(4):
            diag*=master_array[row +i][column + i]
        if diag >= max_product:
            max_product = diag
...
We can then check to see our answer:
```python
print max_product
#70600674
```

That's the correct answer!  Our whole code would then be:
...
#Create the data structure representing the grid
grid_list = grid_str.split()
master_array = []
newlist = []
count = 0
...
In the webcast, Luke also mentioned using a try-except block to solve the problem.  A solution doing that for a grid of arbitrary size is shown below.
