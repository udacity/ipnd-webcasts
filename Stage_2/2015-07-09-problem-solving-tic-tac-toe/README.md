...
Now, for unnamed parameters, we include a single `*` in front of the tuple (or list) to 'unpack' it, like so:
```python
some_list = [1,2,3]
...
A useful link explaining how to use the reverse, generally referred to as `*args` and `**kwargs`, can be found here: http://stackoverflow.com/questions/3394835/args-and-kwargs
...
One easy way to format strings in Python is to use the string.format() method; it takes any number of arguments, named or unnamed, and substitutes them into a given string.  For example:
...
We'll make a function to do this.  As always, we'll think of needed inputs and outputs:
#####Inputs
